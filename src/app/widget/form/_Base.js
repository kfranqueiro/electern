define([
	...
	return declare([ _WidgetBase, _TemplatedMixin ], {
		...
		buildRendering() {
			if (this.partials) {
				this.templateString = string.substitute(this.templateString, this.partials, function (value, key) {
		...
		_getValueAttr() {
			var elements = this.domNode.elements;
			var values = {};
			...
			for (var i = 0; i < elements.length; i++) {
				...
				var value = element[element.type === 'checkbox' ? 'checked' : 'value'];
		...
		_setValueAttr(value) {
			this.domNode.reset();
			var flattenedValue = util.flatten(value);
			for (var key in flattenedValue) {
				var input = this.domNode.elements[key];
				if (input) {
					this._setInputValue(input, flattenedValue[key]);
				}
			}
		},
