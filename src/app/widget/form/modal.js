"use strict";
define(["dojo/Deferred", "dojo/dom-construct", "dojo/on", "./_Confirm"], function (
  Deferred,
  domConstruct,
  on,
  Confirm
  ...
  function findTabbableElements(parentNode) {
    const matches = slice.call(
      parentNode.querySelectorAll("button,input,select,textarea,a[href],[tabindex]")
    );
  ...
  function hide() {
    ...
    if (!z) {
      electronApi.setMenuEnabled(true);
      underlayNode.remove();
  ...
  function handleKeys(event) {
    if (event.keyCode === 9) {
      ...
      if (event.shiftKey && event.target === info.first) {
        ...
        (!event.shiftKey && event.target === info.last)
      ) {
        info.first.focus();
        event.preventDefault();
      }
  ...
  const modal = {
    ...
    show(formWidget) {
      ...
      const modalNode = domConstruct.create(
        "div",
        {
          class: "modal",
          style: { zIndex: z },
