...
define([
  ...
  const userDataDir = electronApi.getUserDataPath("userData");

  feedStore.setFilename(electronApi.resolve(userDataDir, "feeds.json"));
  articleStore.setFilename(electronApi.resolve(userDataDir, "articles.json"));
  ...
  feedGrid.on("dgrid-select", function (event) {
    ...
    if (item.id === "pinned") {
      ...
      collection = searchRx
        ? articleStore.filter(function (article) {
