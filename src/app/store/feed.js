define([
   ...
   */
  const FeedStore = FsMemory.createSubclass([Trackable, Tree], {
    getChildren() {
      const subcollection = this.inherited(arguments);
      subcollection.queryLog = subcollection.queryLog.concat(
        this.queryLog.filter(function (args) {
    ...
    getUnread(item) {
      ...
      return "unread" in item
        ? item.unread
        : this.getFeeds(item).reduce((total, item) => {
    ...
    toJSON() {
      return this.storage.fullData.filter((item) => !item._system);
    },
  });
